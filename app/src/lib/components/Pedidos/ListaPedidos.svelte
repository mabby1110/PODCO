	...
	function handleEdit(elementos: any[]) {
		...
		elementos.forEach((item) => {
			...
			for (let i = 0; i < cantidad; i++) {
				productosSeleccionadosStore.agregar(item.inventario);
			}
		});
	}
		...
		<div class="pedido panel">
			...
			{#each grupo.elementos as item}
				<div class="producto">
					...
					<span>USD</span>
					<span class="cantidad">
						<p>{item.inventario.precio}</p>
					</span>
					<span class="cantidad">
						{item.inventario.cantidad}
					</span>
					<span class="total">
						{formatCurrency(String(item.inventario.precio * item.inventario.cantidad), 'USD')}
					</span>
				</div>
			{/each}
