<script lang="ts" generics="T">
    import { globalFilterStore, type ColumnDef, type FilterAction } from '$lib/stores/globalFilterStore.svelte';
		...
		{#if globalFilterStore.activeFilters.length > 0}
			<div class="active-filters">
				...
				{#if globalFilterStore.activeFilters.length > 1}
				   <button class="chop butter" onclick={() => globalFilterStore.clearFilters()}>Limpiar todo</button>
				{/if}
			</div>
		{/if}
	{:else}
		<button class="butter" onclick={() => (show = true)}>Agregar Filtro ({filterCount})</button>
	{/if}
</div>
