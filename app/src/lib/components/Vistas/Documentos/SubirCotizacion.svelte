<script lang="ts">
    ...
    let totalPedidos = $derived(
        pedidos.reduce((acc: number, item: any) => {
            return item.estatus === 'seleccionado' ? acc + item.precio_unitario * item.cantidad : acc;
        }, 0)
    );
    ...
    async function handleSubmit(event: SubmitEvent) {
        ...
        try {
            ...
            if (result.type === 'success' || result.status === 200) {
                ...
                const pedidosAprobados = pedidos.filter((p) => p.estatus === 'seleccionado');
                
                if (pedidosAprobados.length > 0) {
                    const pedidosACrear = pedidosAprobados.map(({ id, ...p }) => ({
                        ...p,
                        estatus: 'cotizado',
                        id_cotizacion: idDocumento
                    }));
                ...
                {#each items as item, i}
                    <div class="item panel">
                        ...
                        <div class="info">
                            ...
                            <!-- El input ahora usa "value=" en lugar de "bind:value=" y tiene el atributo "readonly" -->
                            <input
                                id="monto"
                                type="number"
                                value={totalPedidos}
                        ...
                        <!-- Se añade "totalPedidos === 0" a la condición para deshabilitar el botón -->
                        <button type="submit" class="butter matcha guardar" disabled={isSubmitting || totalPedidos === 0}>
...
<style>
    ...
    .info input[type='number'] {
        max-width: 250px;
        background-color: var(--color-background-muted, #f3f4f6); /* Feedback visual opcional */
        cursor: not-allowed;
    }
</style>